...
// Shared header/footer chrome for QC sub-pages. Navy bar, breadcrumb
// (Modules / QC / <title>), avatar, and footer — mirrors PurchaseChrome.

import { useRouter } from "next/navigation";
import { BrandMark } from "@/components/BrandMark";
import { useUserInitial } from "@/lib/user";

export interface QcChromeProps {
  title: string;
  children: React.ReactNode;
}

export function QcChrome({ title, children }: QcChromeProps) {
    ...
    <div className="min-h-screen flex flex-col bg-[var(--background)]">
      <header className="bg-[var(--aws-navy)] h-[45px] flex items-center px-6 gap-4">
        ...
        <nav className="text-[12px] text-[#d5dbdb] hidden md:flex items-center gap-2 ml-2">
          ...
          <span>/</span>
          {title === "QC" ? (
            <span className="text-white">QC</span>
          ) : (
            <>
              <button onClick={() => router.push("/modules/qc")} className="hover:underline">QC</button>
              <span>/</span>
